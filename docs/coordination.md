...
| DOC2 GUI 协议文档页 | session-ffc31e23（专属新会话） | feat/gui-protocol-docs | apps/gui/src/**：/docs 路由 + palette-nav 登记 + 设置页入口行 + i18n zh/en（键先独立小提交）；rail 保持 4 项；内容单源 vite raw 引 docs/protocol/ 禁复制 | pnpm build/lint/test/check:i18n 全绿 + vitest 断言真实文档渲染 + MENU_ENTRIES 仍 4 项 + 无新增 Tauri 命令 + diff 域合规 | 已完成：3048c05 ff 合入（860 vitest/i18n 960=960/rail 4 项未破/worktree 全量 make check 绿）；worktree 与分支全清 |
