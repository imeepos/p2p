...
- 参数串 `Noise_XX_25519_ChaChaPoly_SHA256`（:19）；XX 模式双方交换静态钥，无明文阶段。
- X25519 静态钥由同一 ed25519 身份派生：私钥 = SHA512(种子) 前 32 字节做 RFC 7748
  clamp；公钥 = ed25519 公钥转蒙哥马利坐标（:76-92）。
- 三条握手消息（发起方视角）：msg1 发 e；msg2 收 e,s,密文负载；msg3 发 s,密文负载。
- 身份负载定长 96 字节 = 32 字节 ed25519 公钥 || 64 字节 ed25519 签名（:105-115）；
  签名内容 = 域串 `p2p-noise-xx-v1`（:21）拼接本端 X25519 静态公钥。
  长度不符或验签失败即身份不可信，断链（:117-135）。
- 握手帧与传输态记录同格式：2 字节大端长度 + 密文（u16be(len) || ciphertext，:147-176）；
