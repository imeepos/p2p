...
拍板基线（不再开放讨论）：左侧收敛为窄图标栏（icon rail），一级入口 4 个——聊天、通讯录、网络、设置（沉底）；单聊/群聊/agent 会话统一为「聊天」一种心智，聊天是唯一主入口；旧路由全部保留重定向；快捷键重映射到 4 入口；命令面板仍可达全部子页/tab；表单三律（前置校验同口径、禁自由文本、历史值下拉）已拍板，落点见 2.5 与 3.4；破坏性动作确认三档制已拍板，定义见 3.5。
...
- 顶栏（节点状态 pill、启停、主题/语言）与底部状态栏（运行状态、监听端口、活跃连接、版本）保留不动。顶栏同时是 macOS Overlay 沉浸式标题栏的拖拽区（tauri.conf `titleBarStyle: Overlay` + `hiddenTitle`，2026-09-08 双标题栏收敛拍板）：原生标题文字隐藏、应用名仅顶栏一处展示，rail nav 顶部为红绿灯让位。
...
### 3.5 破坏性动作确认三档制（已拍板，全应用适用）

| 档位 | 交互 | 适用动作 |
|---|---|---|
| 第一档（不可逆且丢数据） | 输入确认文本模式：用户须手动输入指定词（如节点 PeerId 短码）才可执行 | 重置身份 |
| 第二档（不可逆但不丢数据） | AlertDialog 二次确认 + 破坏性红色按钮 + 后果明示文案 | 解散群、删好友、踢出成员、删除 agent endpoint（含「将同时移除本地会话记录索引」影响说明，与 3.3 危险区一致） |
| 第三档（可逆或影响小） | 单次确认即可 | 退群（数据仅置位保留）、清理日志/本地缓存 |

落点引用：好友删除与移动（3.1 好友区）、解散群与退群（3.1 群区）、agent 停用/删除（3.3 危险区）、日志/缓存清理（网络页诊断 tab）、重置身份（设置页）。验收归 P2：各危险动作落入正确档位的断言；重置身份输入确认文本断言；第二档红钮与后果文案断言。
...
| `/group` | `/chat?kind=group` | 落聊天并聚焦排序最前的群条目；无群则空态（已拍板项 1） |
...
- **P2**：影响 `views/contacts/**`（新）、迁移 `chat-friend-add-dialog` 等 4 个对话框、`acp/config-panel.tsx`、`permission-panel.tsx`、`capabilities-card.tsx`、i18n。验收：三区空态与行操作断言；加好友 out/in 邀请全流程测试（沿用现 chat-friend-add 测试迁移）；创建群→邀请→入群流程断言；endpoint 添加→测试连接→保存→发起会话断言；详情抽屉五块渲染与权限档变更生效断言；表单三律改造项断言（移动分组下拉 + 新建分组路径、endpoint 历史值下拉、错误码 i18n 快照，3.4）；破坏性动作三档制断言（各动作落对档位、重置身份输入确认文本、第二档红钮与后果文案，3.5）。
...
以下五项已按推荐方案拍板，正文即确定规格：

1. **`/group`、`/acp` 深链落地**：`/chat?kind=group|agent`，聚焦排序最前的对应条目，无对应条目则空态（5.3 重定向表按此执行）。拍板理由：旧页无会话参数语义，聚焦第一条比空列表更可达。
2. **未读计数**：仅内存态（store），不落盘，重启归零（2.3 按此执行）。agent 离线队列语义出现后再评估升级。
3. **agent 条目 `lastPreview`**：transcript 最后一条 user/agent 文本截断 40 字（2.2 按此执行）。
4. **通讯录三区布局**：纵向三分节 + 页顶锚点条（3.1 按此执行）。
5. **概览页保留节点启停按钮**：启停常驻概览状态卡（4.2 按此执行），顶栏 pill 不变。
