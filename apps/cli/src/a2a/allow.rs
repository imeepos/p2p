...
pub async fn run_allow(args: AllowArgs) -> CliResult<()> {
    ...
    if args.json {
        println!(
            "{}",
            json!({ "granted": { "agentId": args.agent_id, "peer": args.peer_id } })
        );
    } else {
...
pub async fn run_disallow(args: DisallowArgs) -> CliResult<()> {
    ...
    let bytes =
        std::fs::read(&grants_file).map_err(|e| CliError::Runtime(format!("read grants: {e}")))?;
    ...
    if args.json {
        println!(
            "{}",
            json!({ "revoked": { "agentId": args.agent_id, "peer": args.peer_id } })
        );
    } else {
