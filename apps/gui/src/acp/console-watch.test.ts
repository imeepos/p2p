...
// 契约 v10 §15（UX3）console-watch 自动流程 mock 覆盖：
// 1) ready -> 本机 agent 端点自动登记（稳定 id + 默认别名）且相位重放幂等（不重复登记、不重复连接）；
// 2) 登记后自动连接 + 自动开新会话（发现面解析 peer）；
// 3) failed/unavailable 相位显式落 store（不静默），恢复 ready 后流程可重入。
...
describe("console-watch 自动登记与零点击直达（契约 v10 §15）", () => {
  it("ready 即自动登记本机 agent（稳定 id/默认别名/console 连接面），发现面解析 peer 后自动连接并自动开会话", async () => {
    stubDiscoveryFetch();
    ensureConsoleWatch();
    emitReady();
    ...
    expect(local!.adminUrl).toBe("http://127.0.0.1:8790");
  ...
  it("相位重放幂等：同 ready 状态重发不产生重复端点、不发起重复连接（无风暴）", async () => {
    stubDiscoveryFetch();
    ensureConsoleWatch();
    emitReady();
  ...
  it("failed/unavailable 相位显式落快照；恢复 ready 后登记连接流程可重入", async () => {
    stubDiscoveryFetch();
    ensureConsoleWatch();
    mockAcpConsole.emit({ phase: "unavailable", restarts: 0 });
    expect(useAcpStore.getState().console).toMatchObject({ phase: "unavailable" });
    expect(useAcpStore.getState().saved).toHaveLength(0);
    mockAcpConsole.emit({ phase: "failed", restarts: 5, lastError: "exit status 1" });
    expect(useAcpStore.getState().console).toMatchObject({ phase: "failed", restarts: 5 });
    // 重启成功（新连接面）：重新登记且完成自动连接
    emitReady();
    await vi.waitFor(() => {
      expect(useAcpStore.getState().activeEndpointId).toBe(LOCAL_AGENT_ENDPOINT_ID);
      expect(useAcpStore.getState().phase).toBe("online");
