...
// - 选中态路由化 ?peer=/?group=/?a2a=；无 query 右侧空态（§2.1）。
// - <768 单栏互斥（防御性规则）：默认显列表，选中切入记录，记录左上返回。
// - ?kind=group|a2a（旧 /group /acp 重定向落点，拍板项 1）：聚焦排序最前
//   的对应条目，无对应条目保持空态。
// ACS2：agent 形态拆除，唯一入口收敛 /agent（views/agent-chat）。
const SELECTION_KEYS = ["kind", "peer", "group", "a2a"] as const;
...
export function ChatPage() {
  ...
  const selectedId = peerParam ?? groupParam ?? a2aParam;

  // 深链落定：选中即清零（1:1/群在 select* 内清，§2.3）。
  useEffect(() => {
    if (!peerParam) return;
  ...
  useEffect(() => {
    if (peerParam || groupParam || a2aParam) return;
    if (kindParam !== "group" && kindParam !== "a2a") return;
    const first = entries.find((e) => e.kind === kindParam);
    if (!first) return;
    const key = first.kind === "group" ? "group" : "a2a";
    setSearchParams(
      (prev) => {
        const next = new URLSearchParams(prev);
        next.delete("kind");
  ...
  }, [kindParam, peerParam, groupParam, a2aParam, entries, setSearchParams]);
