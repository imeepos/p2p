...
export function A2aConversation({ agentKey, hostPeer, agentId, agentName }: A2aConversationProps) {
  ...
  const [taskState, setTaskState] = useState<A2aTaskState | null>(null);
  const [isSending, setIsSending] = useState(false);

  const a2aStore = useA2aStore();
  const task = taskId ? a2aStore.getTask(taskId) : null;
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-hidden">
        {messages.length === 0 ? (
          <EmptyState
            icon={Bot}
            title={t("a2a.empty.title" as any)}
            description={t("a2a.empty.description" as any)}
          />
        ) : (
          <MessageList
            peer={agentKey}
