...
export function isFailedSendReport(report: FailedSendReportLike): boolean {
  return !report.delivered && report.message.status === "failed";
}

export function notifyFailedSendReport(report: FailedSendReportLike): void {
  if (!isFailedSendReport(report)) return;
  console.error(
    "[chat] send failed (mark_failed):",
    report.message.peer ?? "(group)",
    report.message.id ?? "",
  );
