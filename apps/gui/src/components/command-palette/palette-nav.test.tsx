...
describe("面板导航注册表（5.2：17 项 = 4 rail + ACS1 /agent + 1 消息中心 + 1 协议文档 + 6 网络 tab + 3 通讯录锚点 + 1 远程访问）", () => {
  it("注册表恰好 17 项且构成符合拍板基线", () => {
    // ACS1（2026-09-14）：agent 独立会话页登记进面板（与 rail 注册同步），
    // 非网络/非锚点计数 7 -> 8，其余构成不变。
    expect(PALETTE_NAV_ENTRIES).toHaveLength(17);
    expect(
