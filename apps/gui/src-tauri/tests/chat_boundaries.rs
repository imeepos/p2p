use p2p_chat::{
    ChatEnvelope, ChatFriend, ChatKind, ChatMediaMeta, ChatSendReport, ChatStatus, Sender,
    MAX_MESSAGE_SIZE,
};
...
fn decode<T: DeserializeOwned>(v: Value) -> Result<T, serde_json::Error> {
    serde_json::from_value(v)
}
fn envelope() -> ChatEnvelope {
    ChatEnvelope {
        id: "m".into(),
        peer: "peer".into(),
        sender: Sender::Me,
...
fn contract_roundtrips_all_chat_shapes_and_camel_case() {
    ...
    for value in [
        serde_json::to_value(&friend).expect("friend json"),
        serde_json::to_value(&report).expect("report json"),
    ] {
        assert!(
            value.get("peerId").is_some() || value.get("message").is_some(),
            "契约字段必须存在: {value}"
        );
    ...
    assert_eq!(
        value["replyTo"],
        Value::Null,
        "无引用 replyTo 序列化 null（camelCase）"
    );
    assert_eq!(
        value["media"]["path"],
        Value::Null,
...
fn json_missing_null_unknown_and_camel_case_are_explicit() {
    ...
    assert_eq!(
        with_reply.reply_to.as_deref(),
        Some("q-1"),
        "replyTo 映射错误"
    );
    assert!(
        decode::<ChatEnvelope>(json!({"id":"m"})).is_err(),
        "缺必填字段必须失败"
    );
    ...
    let media: ChatMediaInputJson =
        decode(json!({"name":"x","mime":"image/png","dataBase64":"eA==","extra":1}))
            .expect("输入未知字段兼容");
...
fn peer_nickname_text_media_limits_have_readable_errors() {
    assert!(p2p_chat::validate_text(" ")
        .expect_err("空文本必须拒绝")
        .to_string()
        .contains("文本"));
    assert!(p2p_chat::validate_text(&"x".repeat(2001))
        .expect_err("超长文本必须拒绝")
        .to_string()
        .contains("上限"));
    assert_eq!(p2p_chat::validate_text(" x ").expect("合法文本"), "x");
    assert!(p2p_chat::validate_media(&ChatKind::Image, "text/plain", 1)
        .expect_err("MIME 错误")
        .to_string()
        .contains("MIME"));
    assert!(p2p_chat::validate_media(
        &ChatKind::File,
    ...
    .contains("64MiB"));
    assert_eq!(
        p2p_chat::sanitize_name("../../a b.txt"),
        "....ab.txt",
        "文件名边界应可观测"
    );
}
...
fn asset_url_encodes_path_and_uses_platform_prefix() {
    ...
    assert!(
        url.contains("%2F") && url.contains("%20") && url.contains("%E4%B8%AD"),
        "路径必须按 URI component 编码: {url}"
    );
    assert!(
        !url.trim_start_matches("asset://localhost/").contains('/'),
        "编码路径不得残留裸斜杠: {url}"
    );
}
