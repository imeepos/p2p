...
fn save_list_get_remove_roundtrip() {
    ...
    assert_eq!(
        get(dir.to_str().unwrap(), "prov-a").unwrap().unwrap().name,
        "A"
    );
    assert!(remove(dir.to_str().unwrap(), "prov-a").unwrap());
    assert!(
        !key_path(dir.to_str().unwrap(), "prov-a").exists(),
        "级联删密钥文件"
    );
...
fn model_conflict_between_providers_rejected() {
    ...
    let err = save(
        dir.to_str().unwrap(),
        params(Some("prov-b"), "B", &["gpt-4o", "claude-3"]),
    )
    .unwrap_err();
...
fn required_fields_are_validated() {
    ...
    .is_err());
    assert!(
        list(dir.to_str().unwrap()).unwrap().is_empty(),
        "失败路径零落盘"
    );
    let _ = std::fs::remove_dir_all(&dir);
}
