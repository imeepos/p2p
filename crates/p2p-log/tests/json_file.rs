...
fn json_format_writes_parseable_events() {
    ...
    assert!(content.contains("json event marker"), "事件文本必须落盘: {content}");
    for line in content.lines().filter(|l| !l.trim().is_empty()) {
        let trimmed = line.trim_start();
        assert!(trimmed.starts_with('{'), "每行必须是 JSON 对象: {line}");
        assert!(line.trim_end().ends_with('}'), "每行必须是完整 JSON 对象: {line}");
        assert!(line.contains(r#""level":"INFO""#), "JSON 必须带 level 字段: {line}");
    }
