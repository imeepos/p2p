...
fn expected_tag(name: &str) -> u64 {
    match name {
        ...
        n if n.starts_with("reject_code_") => {
            let code: u64 = n.trim_start_matches("reject_code_").split('_').next().unwrap().parse().unwrap();
...
fn golden_bytes_roundtrip_all_tags() {
    ...
    for case in cases(&doc) {
        ...
        let kind = msg.kind.as_ref().unwrap_or_else(|| panic!("case {name}: 缺 kind"));
        assert_eq!(kind_tag(kind), expected_tag(name), "case {name}: oneof tag 不符");
        assert_eq!(
            msg.encode_to_vec(),
            protobuf,
...
async fn framed_vectors_read_back_via_read_msg() {
    ...
    for case in cases(&doc) {
        let name = case_name(case);
        let frame = unhex(case["frame_hex"].as_str().unwrap());
        let expect = RelayMsg::decode(unhex(case["protobuf_hex"].as_str().unwrap()).as_slice()).unwrap();
...
fn constructors_reproduce_golden_bytes() {
    ...
    for case in cases(&doc) {
        ...
        let rebuilt = match name {
            ...
            "punch_ack" => RelayMsg::punch_ack("peer-a", vec![
                "198.51.100.9:7777".into(),
                "[2001:db8::1]:4444".into(),
            ]),
        ...
        assert_eq!(rebuilt.encode_to_vec(), protobuf, "case {name}: 构造器编码不符");
    }
}
