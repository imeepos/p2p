...
fn identity_payload_chain_matches_vector() {
    ...
    let seed: [u8; 32] = unhex(case["seed_hex"].as_str().unwrap()).try_into().unwrap();
    let kp = Keypair::from_seed(&seed);
    let pubkey = kp.public();

    assert_eq!(
        p2p_conformance::hex(&pubkey),
        case["public_key_hex"].as_str().unwrap(),
        "ed25519 公钥不符"
    );
    assert_eq!(
        p2p_conformance::hex(&x25519_of(&pubkey)),
    ...
    assert_eq!(
        p2p_conformance::hex(&sig),
        case["signature_hex"].as_str().unwrap(),
        "签名不符（ed25519 确定性）"
    );

    let payload = unhex(case["identity_payload_hex"].as_str().unwrap());
    assert_eq!(payload.len(), case["identity_payload_len"].as_u64().unwrap() as usize, "负载长度须为 96");
    assert_eq!(&payload[..32], &pubkey[..], "负载 [0..32) 必须是 ed25519 公钥");
    assert_eq!(&payload[32..], &sig[..], "负载 [32..96) 必须是签名");
    assert!(Keypair::verify(&pubkey, &signed, &sig), "黄金负载必须验签通过");
}
...
fn wrong_domain_fails_verification() {
    let case = cases(&load(FILE))
        ...
        .clone();
    let pubkey: [u8; 32] = unhex(case["public_key_hex"].as_str().unwrap()).try_into().unwrap();
    let signed = unhex(case["signed_message_hex"].as_str().unwrap());
    let sig: [u8; 64] = unhex(case["signature_hex"].as_str().unwrap()).try_into().unwrap();
    // 被签消息域名不同（p2p-noise-xx-v2），同一签名必须验不过
    assert_ne!(&signed[..15], b"p2p-noise-xx-v1");
    assert!(!Keypair::verify(&pubkey, &signed, &sig), "异域串验签必须失败");
}
...
fn identity_cert_carries_vector_oid_and_pubkey() {
    ...
    let seed: [u8; 32] = unhex(golden_case()["seed_hex"].as_str().unwrap()).try_into().unwrap();
    let kp = Keypair::from_seed(&seed);
    ...
    assert_eq!(ext.extn_value.as_bytes(), &kp.public()[..], "扩展内容必须是 32 字节公钥");
}
