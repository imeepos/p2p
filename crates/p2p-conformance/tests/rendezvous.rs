...
fn golden() -> (Keypair, rv::Register, Vec<u8>, u64) {
    ...
    assert_eq!(
        p2p_conformance::hex(&reg.encode_to_vec()),
...
fn register_golden_matches_implementation() {
    ...
    assert_eq!(reg.sig, kp.sign(&rv::signed_payload(
        &reg.namespace,
        &PeerId::from_bytes(reg.peer_id.clone().try_into().unwrap()),
        &reg.addrs.iter().map(|a| a.to_addr().unwrap()).collect::<Vec<_>>(),
        reg.ttl_secs,
        reg.issued_at,
    )));
...
fn ttl_tamper_breaks_signature() {
    ...
    assert!(!rv::verify_register(&reg, issued_at), "改 TTL 后验签必须失败");
}
...
fn signature_bit_flip_breaks_verification() {
    ...
    reg.sig[0] ^= 0x01;
    assert!(!rv::verify_register(&reg, issued_at), "翻转签名一字节后必须失败");
    let mut signed = rv::signed_payload(&reg.namespace, &kp.peer_id(), &reg.addrs.iter().map(|a| a.to_addr().unwrap()).collect::<Vec<_>>(), reg.ttl_secs, reg.issued_at);
    signed[0] ^= 0x01;
    let sig: [u8; 64] = reg.sig.clone().try_into().unwrap();
    assert!(!Keypair::verify(&kp.public(), &signed, &sig), "翻转被签字节后必须失败");
}
