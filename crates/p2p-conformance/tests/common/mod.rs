//! 符合性向量消费公共件（tests/ 路径，panic-hygiene 门禁豁免）。
//!
//! 向量唯一源是 docs/protocol/vectors/*.json；经 CARGO_MANIFEST_DIR
...
// 各测试二进制只取所需 helper，未取项按测试基建惯例豁免 dead_code。
#![allow(dead_code)]
...
pub fn load(file: &str) -> Value {
    ...
    let raw = fs::read_to_string(&path)
        .unwrap_or_else(|e| panic!("向量文件不可读 {}: {e}", path.display()));
