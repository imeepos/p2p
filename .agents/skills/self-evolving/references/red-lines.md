...
- 2026-09-05 禁止从终端回显复制密钥/长凭据进任何存储——无尾换行文件 cat 后 zsh 会补显 EOL 标记 %，复制必带尾巴（本次签名 secret 四平台发布失败根因）；一律文件重定向或 API 传原文。
- 2026-09-06：合并失败的分支清理绝不能与合并命令串在同一执行序列里连环跑——ff-only 分叉失败后，后续 worktree remove/branch -d 仍会执行，把未合并分支连本地带远端一起删掉；每一步必须核验上一步结果（本次靠已知哈希 92621f3 + 对象库未 GC 才零损失恢复）。
- 2026-09-06 UX1：主树全量终验（make check 等）运行期间是工作区改写冻结窗——哪怕只是 docs(skill) 反思喂回也必须让行（并行会话基线「测试运行与工作区改写严禁对同一 worktree 并发」适用于一切写入）；用 schedule 定时器把写入推迟到终验结束后是合法让行姿势，发现共享树有他人在途改动时只写干净文件并通报，绝不吞并。
...
## 禁止直推 origin/main 任何写入（含 docs(skill) 提交）——A2-ACP 轮负责人裁定（2026-09-09）
...
- 禁止把公网入口运行手册（域名、端口、认证文件位置、拓扑、DNS 记录号）提交进 `imeepos/p2p`：`curl -s https://api.github.com/repos/imeepos/p2p` 已确认 `"private": false`，而 dsh web 是「可执行命令 + 读写本机文件」的 agent GUI，手册等于给扫描器递地图。运维手册落仓库外（`~/ext512/ops-notes/`），仓库内只留不含入口的机理经验；公开仓库里写经验前先判断这句话会不会被当成入口线索（2026-09-18 dsh.veren.top 轮裁定）。
