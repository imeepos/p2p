...
- 后台跑长驻服务（vite dev 等）严禁管道接 `| head/-tail`：head 读满退出后 SIGPIPE 杀死服务进程，表现为 job completed 但端口无人监听；应输出重定向到日志文件（`> /tmp/x.log 2>&1`），用 curl 探活。
- 本机 5173 可能被其他项目 dev server 常驻（cms/web 等），起 vite 用 `--port <冷门口> --strictPort` 显式占位，避免「探测 5173 连上了别人的应用」误判。
